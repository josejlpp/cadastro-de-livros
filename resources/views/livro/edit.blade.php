...
@section('subtitle', 'Livro - Edição')
@section('content_header_title', 'Edição')
...
@section('content_body')
    <form action="{{ route('livro.update', $livro->Codl) }}" method="POST">
        ...
        <div class="row">
            <x-adminlte-input name="ano_publicacao" value="{{ old('ano_publicacao', $livro->AnoPublicacao) }}" label="Ano de publicação" placeholder="Ex: 1996"
                              fgroup-class="col-md-12"/>
        </div>
        <div class="row">
            <x-adminlte-input id="valor" name="valor" value="{{ old('valor', $livro->Valor) }}" label="Valor" placeholder="Valor do livro"
            ...
            <label for="autor_id" class="col-md-6">
                ...
                <select class="select2 form-control" name="autor_id[]" multiple="multiple">
                    ...
                    @foreach($autores as $autor)
                        <option {{ in_array($autor->CodAu, old('autor_id', $livro->autores->pluck('CodAu')->all())) ? 'selected' : '' }}
                                value="{{ $autor->CodAu }}">
                            {{ $autor->Nome }}
                        </option>
                    @endforeach
            ...
            <label for="" class="col-md-6">
                ...
                <select class="select2 form-control" name="assunto_id[]" multiple="multiple">
                    ...
                    @foreach($assuntos as $assunto)
                        <option {{ in_array($assunto->CodAs, old('assunto_id', $livro->assuntos->pluck('CodAs')->all())) ? 'selected' : '' }}
                                value="{{ $assunto->CodAs }}">
                            {{ $assunto->Descricao }}
                        </option>
                    @endforeach
